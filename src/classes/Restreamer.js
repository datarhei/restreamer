...
class Restreamer {
    ...
    static fetchSnapshot (firstSnapshot) {
        if (Restreamer.data.states.repeatToLocalNginx.type === 'connected' || firstSnapshot) {
            ...
            command.on('end', () =>{
                ...
                Q.delay(process.env.SNAPSHOT_REFRESH_INTERVAL).then(() => {
    ...
    static restoreFFMpegProcesses () {
        ...
        if (Restreamer.data.addresses.srcAddress &&
            (Boolean(repeatToLocalNginxIsConnected) ||
            Boolean(repeatToLocalNginxIsConnecting))) {
        ...
        if (Restreamer.data.addresses.optionalOutputAddress &&
            (Boolean(repeatToOptionalOutputIsConnected) ||
            Boolean(repeatToOptionalOutputIsConnecting))) {
